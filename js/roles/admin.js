...
//del archivo administracion de buses (vehicle.js) que requieren un tamano de entre 2 y 45 caracteres
        ...
        camposAcronimo: function (val, field) {
            if (!/^[A-Z.a-z]{1}[-0-9.A-Z.a-z.áéíóúñ\s*]{2}$/.test(val)) {
                return false;
            }
            return true;
        },
        camposAcronimoText: 'El primer caracter deberá ser una LETRA <br> Solo caracteres alfanumericas <br>  el tamaño deberá ser de 3 caracteres',
...
//para numeros 3-45
            ...
            click: function (menu, item, e, eOpts) {
                for (var i = 0; i < showCoopMap.length; i++) {
                    if (showCoopMap[i][0] === item.getItemId()) {
                        ...
                        if (!item.checked) {
                            console.log("Se muestran los equipos: " + showCoopMap[i][0]);
    ...
    var administracion = Ext.create('Ext.button.Button', {
        height: 30,
        text: '<span class="btn-menu">Administración</span>',
        iconCls: 'icon-direccion',
        tooltip: 'Administración de Distritos, Usuarios, Personas y Vehículos',
        scope: this,
        menu: [
            {text: 'UPC', iconCls: 'icon-central', handler: function () {
    ...
    var barraMenu = Ext.create('Ext.toolbar.Toolbar', {
        ...
        items: [{
                height: 30,
                text: '<span class="btn-menu">Reportes</span>',
    ...
    var panelMenu = Ext.create('Ext.form.Panel', {
        ...
        items: [{
                ...
                items: [{
                        xtype: 'label',
                        html: '<img src="img/123.PNG" width="100%" height="100">'
                    },
//                    {
//                        xtype: 'label',
//                        padding: '15 0 0 0',
        ...
        items: [
            {
                ...
                title: '<div id="titulo">Distritos</div>',
                autoScroll: true,
                iconCls: 'icon-tree-company',
                store: storeTreeVeh,
                columns: [
                    {xtype: 'treecolumn', text: 'Bases de UPC’s', flex: 4, sortable: true, dataIndex: 'text'}
                ],
                tools: [{
                        type: 'help',
                        handler: function () {
...
//            },
    ...
    var gridEventos = Ext.create('Ext.grid.GridPanel', {
        region: 'south',
        title: "<center>INTEGRANTES</center>",
        collapsible: true,
        collapsed: true,
        split: true,
        height: 200,
        autoScroll: true,
        frame: true,
        store: storePersonas1,
        features: [filters],
        columns: [
            {header: "Cedula", flex: 75, sortable: true, dataIndex: "cedula"},
            {header: "Nombres", flex: 75, sortable: true, dataIndex: "nombres"},
            {header: "Apellidos", flex: 75, sortable: true, dataIndex: "apellidos", filter: {type: 'string'}},
            {header: "Celular", flex: 75, sortable: true, dataIndex: "celular", filter: {type: 'string'}},
            {header: "Correo", flex: 125, sortable: true, dataIndex: "correo", filter: {type: 'list', store: storeEventos}},
        ],
          });
